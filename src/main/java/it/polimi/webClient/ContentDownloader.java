...
public class ContentDownloader {
	...
	private static final int SLEEP_AFTER_REQUEST = 1500;
	...
	public void setupProxyWithCredentials(String proxyHostname, int proxyPort, String username, String password) {
		...
		client.getCredentialsProvider().setCredentials(new AuthScope(proxyHostname, proxyPort), new UsernamePasswordCredentials(username, password));
	}
	...
	private HttpEntity getContent(HttpHost targetHost, String content) throws DownloadException, IOException {
		...
		if (response.getStatusLine().getStatusCode() != RESPONSE_OK) {
			request.abort();
			throw new DownloadException(targetHost.toHostString() + content, response.getStatusLine().getStatusCode());
		}
		...
		if (entity != null) {
			...
			throw new DownloadException(targetHost.toHostString() + content);
		}
	}
